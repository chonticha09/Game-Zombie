...
func _ready():
	...
	texture_rect_nodes.append(get_node("ไม้องุ้น2"))
...
func load_texture(path: String, texture_rect: TextureRect) -> void:
	var texture = preload("res://ร้านผลไม้/องุ่นเสีอบไม้.png")
	if texture:
...
func _on_button_pressed():
	...
	if current_texture_index < texture_rect_nodes.size():
		...
		load_texture("res://ร้านผลไม้/องุ่นเสีอบไม้.png", current_texture_rect)
